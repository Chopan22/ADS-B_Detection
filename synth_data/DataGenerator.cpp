...
int main() {
    ...
    file << "time,icao24,lat,lon,velocity,heading,vertrate,callsign,onground,alert,spi,squawk,baroaltitude,geoaltitude,lastposupdate,lastcontact,target_score\n";
    ...
    for (int i = 0; i < SAMPLES; ++i) {
        ...
        // Write to CSV
        file << p.time << ",4ca765," << std::fixed << std::setprecision(8) 
             << oLat << "," << oLon << "," << std::setprecision(2) 
             << oVel << "," << p.head << "," << oVR << ",BOUND_TEST,False,False,False,1000,"
             << oAlt << "," << oAlt + 50 << "," << p.time - 0.5 << "," << p.time << "," << score << "\n";
        ...
        if (p.head >= 360.0) p.head -= 360.0;
        if (p.head < 0.0) p.head += 360.0;
    }
    return 0;
}
