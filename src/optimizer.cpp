...
void printUsage(const char* progName) {
    ...
    std::cout << "  --output FILE      Output file for results (default: results/optimized_params.txt)\n";
    std::cout << "\nExample:\n";
...
void saveOptimizedParameters(const std::string& filename, 
    ...
    out << "# Training   - MSE: " << trainMetrics.mse 
        << ", F1: " << trainMetrics.f1_score() << "\n";
    out << "# Validation - MSE: " << valMetrics.mse 
        << ", F1: " << valMetrics.f1_score() << "\n\n";
    ...
    for (size_t i = 0; i < best.genes.size(); ++i) {
        ...
        if ((i + 1) % 13 == 0 && i < 51) out << "\n";
        else if ((i + 1) == 59 || (i + 1) == 66) out << "\n";
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "  Train/Val:      " << (trainSplit * 100) << "% / " 
              << ((1.0 - trainSplit) * 100) << "%\n";
    ...
    try {
        ...
        auto baselineTrainMetrics = analysis::Validator::evaluate(
            trainInputs, trainOutputs, defaultChromosome);
        auto baselineValMetrics = analysis::Validator::evaluate(
            valInputs, valOutputs, defaultChromosome);
        ...
        auto optTrainMetrics = analysis::Validator::evaluate(
            trainInputs, trainOutputs, bestChromosome);
        auto optValMetrics = analysis::Validator::evaluate(
            valInputs, valOutputs, bestChromosome);
        ...
        std::cout << std::setw(20) << std::left << "Train F1:"
                  << std::setw(12) << std::fixed << std::setprecision(4) 
                  << baselineTrainMetrics.f1_score()
                  << std::setw(12) << optTrainMetrics.f1_score()
                  << std::setw(10) << std::setprecision(2) << trainImprovement << "%\n";
        
        std::cout << std::setw(20) << std::left << "Val F1:"
                  << std::setw(12) << std::setprecision(4) << baselineValMetrics.f1_score()
                  << std::setw(12) << optValMetrics.f1_score()
                  << std::setw(10) << std::setprecision(2) << valImprovement << "%\n";
        
        std::cout << "\n" << std::setw(20) << std::left << "Train MSE:"
                  << std::setw(12) << std::setprecision(4) << baselineTrainMetrics.mse
                  << std::setw(12) << optTrainMetrics.mse << "\n";
        
        std::cout << std::setw(20) << std::left << "Val MSE:"
                  << std::setw(12) << baselineValMetrics.mse
        ...
        analysis::Validator::saveDetailedResults(
            trainInputs, trainOutputs, valInputs, valOutputs,
            baselineTrainMetrics, baselineValMetrics,
            optTrainMetrics, optValMetrics,
            bestChromosome);
