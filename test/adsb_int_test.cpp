...
public:
  static ValidationMetrics evaluate(const std::vector<std::map<std::string, double>>& inputs,
                                    ...
                                    const std::vector<double>& predicted, double threshold = 0.5) {
  ...
  static void printMetrics(const std::string& label, const ValidationMetrics& m) {
    ...
    std::cout << "  Actual Neg: " << std::setw(12) << m.true_negatives << "  " << std::setw(12)
              << m.false_positives << "\n";
    std::cout << "  Actual Pos: " << std::setw(12) << m.false_negatives << "  " << std::setw(12)
              << m.true_positives << "\n";
    std::cout << "\n";
  }
};

std::vector<double> evaluateFuzzySystem(const std::vector<std::map<std::string, double>>& inputs,
...
void saveResults(const std::string& filename, const ga::Chromosome& best,
                 const ValidationMetrics& trainMetrics, const ValidationMetrics& valMetrics) {
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    auto baselineTrainMetrics =
        RealAdsbValidator::evaluate(trainInputs, trainOutputs, defaultTrainPred);
    auto baselineValMetrics = RealAdsbValidator::evaluate(valInputs, valOutputs, defaultValPred);
    ...
    auto optTrainMetrics = RealAdsbValidator::evaluate(trainInputs, trainOutputs, optTrainPred);
    auto optValMetrics = RealAdsbValidator::evaluate(valInputs, valOutputs, optValPred);
    ...
    std::cout << "  Train F1:         " << std::setw(8) << baselineTrainMetrics.f1_score() << "    "
              << std::setw(8) << optTrainMetrics.f1_score() << "    " << std::setw(8)
              << trainImprovement << "%\n";
    std::cout << "  Val F1:           " << std::setw(8) << baselineValMetrics.f1_score() << "    "
              << std::setw(8) << optValMetrics.f1_score() << "    " << std::setw(8)
              << valImprovement << "%\n";
